...
fn setup(mut commands: Commands) {
    ...
    commands.add(promise!(|s, time: Res<Time>| {
        let t = time.elapsed_seconds();
        info!("start with 31, started at {}, start time stored in state.", t);
        s.with(|_| t).ok(31)
    }).then(promise!(|s, r| {
        ...
        s.ok(())
    })).then(promise!(|s, _, time: Res<Time>, mut exit: EventWriter<AppExit>| {
        info!("Done, time to process: {} (start time took from state {}", time.elapsed_seconds() - s.0, s);
        exit.send(AppExit);
        s.ok(())
    })));
}